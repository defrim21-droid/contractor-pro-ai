...
export const CONSTRAINTS: Record<string, string> = {
  ...
  new_build_masked:
    'Treat all masked regions as unfinished construction substrate. Apply the requested material as an exterior finish system conforming exactly to the existing surface geometry without modifying structural form. ' +
    ...
    'Apply the finish system only within the masked region representing the installation area. Do not extend material beyond mask boundaries. ' +
  ...
  architectural_drawing_masked:
    ...
    'Apply the selected finish system only within the masked region corresponding to the installation area. Do not extend material beyond masked boundaries. ' +
...
export function buildPrompt(
  ...
  if (regionContext) {
    ...
    instruction = `Apply ${regionContext.sampleName} to this masked region. ${colorHint}User instruction: ${instruction}`;
  }
  if (constraint) {
